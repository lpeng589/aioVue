<template>
	<div class="page_salesQuoteAdd">
		<mt-header title="添加销售报价" fixed>
   		 	<div slot="left">
     			 <mt-button icon="back" @click="retSalseQuoteListPage()">返回</mt-button>
   			 </div>
		  ...
		  <mt-navbar v-model="selected">
		    ...
		    <mt-tab-item id="2">报价明细</mt-tab-item>
		  </mt-navbar>
		  <mt-tab-container v-model="selected">
			  <mt-tab-container-item id="1" class="tblOrder">
			   ...
			   <a data-v-4f7ac5e4="" class="mint-cell mint-field" @click="selectClient()">
			    ...
			    <mt-field  label="报价金额" placeholder="请输入金额" v-model="TotalAmount"></mt-field>
			  ...
			  <mt-tab-container-item id="2" class="tblOrderDet">
			  	...
			  	<div class="addGood" @click="$router.push({name: 'goodList',params: {retPage: 'SalesQuoteAdd'}})"><img src="../../assets/goto.png"></div>		  
			  </mt-tab-container-item>
		  </mt-tab-container>
  		</div>
	</div>
...
export default {
  ...
  methods: {
  	...
  	selectClient() {
  		this.$router.push({name: 'clientList', params: {retPage: 'SalesQuoteAdd'}})
  	},
  	retSalseQuoteListPage() {
  		this.saveOrderPre()
  		//console.log(this.DetGoodsCodeStr)
  	    this.$router.push({ name: 'salesQuoteList' })	  
  	},
  	...
  	delDet(index,item) {
  		this.TotalAmount -= this.TABLENAME_tblSalseQuoteDet[index].TaxAmount
