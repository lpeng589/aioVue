<template>
	<div class="page_salesQuoteDetail">
		<mt-header title="销售报价详情" fixed>
   		 	<div slot="left">
     			 <mt-button icon="back" @click="retSalesQuoteListPage()">返回</mt-button>
   			 </div>
		  ...
		  <mt-tab-container v-model="selected">
			  <mt-tab-container-item id="1" class="tblOrder">
			    ...
			    <mt-field  label="报价金额" placeholder="请输入金额" v-model="TotalTaxAmount" readonly="true"></mt-field>
			   ...
			   <mt-tab-container-item id="2" class="tblOrderDet">
			  	...
			  	<div class="good_item" v-for="(item,index) in TABLENAME_tblSalseQuoteDet">
			  		<div class="goodNum">
			  			<p>产品{{ index + 1 }}</p>
			  	...
			  	<div class="addGood" v-show="updateOrderFlag"  @click="$router.push({name: 'goodList',params: {retPage: 'salesQuoteDetail'}})">
...
export default {
  ...
  methods: {
  	...
  	retSalesQuoteListPage() {
  		this.updateOrderFlag = false
  		this.DetGoodsCodeStr = ''
  		this.TABLENAME_tblSalseQuoteDet = []
  		//console.log('retSalesQuoteListPage')
  	    this.$router.push({ name: 'salesQuoteList', params: { opt: 'reload'} })
      ...
      let vm = this
        ...
        this.detail({tableName: this.tableName, keyId: this.keyId}).then(data => {
      	...
      	let DetArr = data.values.TABLENAME_tblSalseQuoteDet
      	...
      	this.TABLENAME_tblSalseQuoteDet = DetArr
        ...
        this.TABLENAME_tblSalseQuoteDet = data.values.TABLENAME_tblSalseQuoteDet
    ...
    delDet(index,item) {
    	if(this.updateOrderFlag){
  			this.TotalAmount -= this.TABLENAME_tblSalseQuoteDet[index].Amount
  			this.TotalTaxAmount -= this.TABLENAME_tblSalseQuoteDet[index].Amount
  			this.TABLENAME_tblSalseQuoteDet.splice(index,1)
  			...
  			goodArr.forEach((val,index) => {
  				if(self.DetGoodsCodeStr.indexOf(val) == -1) {
	  				...
	  				self.popupSelect(cond).then((obj) => { // 根据客户级别取对应的商品价格
	  					...
	  					self.TABLENAME_tblSalseQuoteDet.push(Det)
