<template>
	<div class="page_BuyAskPriceDetail">
		<mt-header title="采购询价详情" fixed>
   		 	<div slot="left">
     			 <mt-button icon="back" @click="retBuyAskPriceListPage()">返回</mt-button>
   			 </div>
   			 <div slot="right" v-show="!updateOrderFlag" @click="updateOrder()">
				<mt-button>
					修改			
				</mt-button>
			</div>
		  ...
		  <mt-navbar v-model="selected">
		    <mt-tab-item id="1">单据信息</mt-tab-item>
		    <mt-tab-item id="2">商品明细</mt-tab-item>
		  </mt-navbar>
		  <mt-tab-container v-model="selected">
			  <mt-tab-container-item id="1" class="tblOrder">
			    <mt-field  label="单据编号" placeholder="请输入编号" v-model="BillNo" readonly="true"></mt-field>
			    <mt-field  label="单据日期" placeholder="请输入日期" v-model="BillDate" readonly="true"></mt-field>
			    ...
			    <mt-field  label="询价金额" placeholder="请输入金额" v-model="TotalTaxAmount" readonly="true"></mt-field>
			   ...
			   <mt-tab-container-item id="2" class="tblOrderDet">
			  	...
			  	<div class="good_item" v-for="(item,index) in TABLENAME_tblBuyAskPriceDet">
			  		<div class="goodNum">
			  			<p>产品{{ index + 1 }}</p>
			  	...
			  	<div class="addGood" v-show="updateOrderFlag"  @click="$router.push({name: 'goodList',params: {retPage: 'BuyAskPriceDetail'}})">
...
export default {
  ...
  methods: {
  	...
  	retBuyAskPriceListPage() {
  		this.updateOrderFlag = false
  		this.DetGoodsCodeStr = ''
  		this.TABLENAME_tblBuyAskPriceDet = []
  		//console.log('retBuyAskPriceListPage')
  	    this.$router.push({ name: 'BuyAskPriceList', params: { opt: 'reload'} })
      ...
      let vm = this
        ...
        this.detail({tableName: this.tableName, keyId: this.keyId}).then(data => {
      	...
      	let DetArr = data.values.TABLENAME_tblBuyAskPriceDet
      	...
      	this.TABLENAME_tblBuyAskPriceDet = DetArr
        ...
        this.TABLENAME_tblBuyAskPriceDet = data.values.TABLENAME_tblBuyAskPriceDet
    ...
    delDet(index,item) {
    	if(this.updateOrderFlag){
  			this.TotalAmount -= this.TABLENAME_tblBuyAskPriceDet[index].Amount
  			this.TotalTaxAmount -= this.TABLENAME_tblBuyAskPriceDet[index].Amount
  			this.TABLENAME_tblBuyAskPriceDet.splice(index,1)
  			...
  			goodArr.forEach((val,index) => {
  				if(self.DetGoodsCodeStr.indexOf(val) == -1) {
	  				...
	  				self.popupSelect(cond).then((obj) => { // 根据客户级别取对应的商品价格
	  					...
	  					self.TABLENAME_tblBuyAskPriceDet.push(Det)
