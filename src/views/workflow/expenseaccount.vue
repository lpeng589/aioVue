...
export default {
  ...
  methods: {
    ...mapActions(['addPre', 'add', 'update', 'popupSelectInfo', 'popupSelect', 'getEnum', 'detail', 'cancelTo', 'hurryTransFunc']),
    ...
    fetchData () {
      ...
      if (this.opType === 'add') { // 新增时拉取数据
        ...
        // 图片处理 ------------------start-------------------
        var BaseUrl =  'http://119.29.172.29:8008'

        let tblName = ''
        fieldList.forEach((obj, index) => {

          tblName = obj.tableName
          if (obj.fieldName == 'tupian' && values.tupian.length > 0) {

            let picName = values.tupian.split(";")
            ...
            picName.forEach((pic, index) => {
             // let url = '/ReadFile.jpg?fileName=' + pic + '&realName=' + pic + '&text-alignmpFile=false&type=PIC&tableName=' + tblName
              let url = BaseUrl+'/ReadFile.jpg?fileName=' + pic + '&realName=' + pic + '&text-alignmpFile=false&type=PIC&tableName=' + tblName
              let picObj = { 'picName': picName, 'url': url }
              picArr.push(picObj)
            })
            vm.$data.picUrl = picArr
           // console.log(vm.$data.picUrl)
         }
        ...
        let fujianName = values.Attachment.split(";")
    ...
    '$route' (to, from) {
      ...
      if (from.name === 'popupEmployee') {
        if (to.params.checked) {
          ...
          window.console.log(checkedEmp)
          this.EmployeeCode = checkedEmp.EmployeeID
