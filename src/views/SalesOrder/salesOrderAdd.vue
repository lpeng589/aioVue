			  ...
			  <mt-tab-container-item id="2" class="tblOrderDet">
			  	<div class="good_item" v-for="(item,index) in TABLENAME_tblSalesOrderDet">
			  		...
			  		<ul>
			  			...
			  			<li class="goodprice" v-if="stocks" @click="$router.push({name: 'stockList',params: {index1: index }})">
			  				<label>仓库</label>
			  				<div>
			  				  <span>{{ item.StockFullName }}</span>
			  				</div>
...
export default {
  ...
  computed: {
    ...
    stockclassstock (){				//接受stockList 参数classCode
    	return this.$route.params.classCode
    },
    index2 (){				//接受stockList 参数index1
    	return this.$route.params.index1
    },
    queryStockByCode_Alert () { // 根据 code 查仓库弹窗
    	return 'model_queryStockByCode' 
    },
    queryStocksOf_Alert() {		//查看多仓库是否开启
    	return 'model_StocksStart'
    }
  },
  data () {
   return {
  ...
  watch: {
  	...
  	$route() {
  		this.stocks_if()
  		this.initSaleStocksDet() //初始化仓库明细
  ...
  methods: {
  	...
  	stocks_if(){
  		console.log(this.stocks)
  			var self = this
  		  	let  cond1= {									//这是获得多仓库是否开启 弹出窗的名字
  				selectName: self.queryStocksOf_Alert,
  			}
  			self.popupSelect(cond1).then((obj) => {
  				//console.log(obj)
  				if(obj[0].Setting!=null&&obj[0].Setting!="false"){
  					console.log("comin")
  					self.stocks=self.stockss
  				}
  				// self.stocks=obj[0].Setting					//如果等于true v-show就会显示多仓库选择
  				// console.log(123)
  				 console.log(self.stocks)
  			})
  	},
  	initSaleStocksDet(){
  		if(this.$route.params.classCode!=null&&this.$route.params.classCode!=undefined&&this.$route.params.classCode!=''){
			let stockArr =this.$route.params.classCode      //把获得仓库Code/仓库名字  赋值  在处理
			let index2 = this.index2						//把获得商品下标 赋值
			//console.log(12121)
			//console.log(stockArr)
		    console.log(index2)
			console.log(this.$route.params.classCode)
			this.stockCode=stockArr
			


			this.array=this.stockCode.split("/")			//将传回来的stockCode分割成  array[0]:code  array[1]:name
			//console.log(self.array)
			this.TABLENAME_tblSalesOrderDet[index2].StockFullName=this.array[1]		//仓库名字赋值
			this.TABLENAME_tblSalesOrderDet[index2].ClassCode=this.array[0]		//仓库classCode赋值
			//console.log(self.TABLENAME_tblSalesOrderDet[0].StockFullName)		//打印仓库名字	
			
			console.log(this.TABLENAME_tblSalesOrderDet)
			//this.saveOrderPre()
		  }	
  	}
  },
